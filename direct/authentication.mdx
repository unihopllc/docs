...
description: 'Sign your requests with an RS256 JWT'
---
...
|-------|-------------|----------|--------|
| `iss` | Issuer - Your partner_uid (provided by us) | Yes | `"iss": "your_partner_uid"` |
| `iat` | Issued At - Current timestamp (seconds since epoch) | Yes | `"iat": 1686104400` |
| `exp` | Expiration Time - Must be within 30 minutes of `iat` | Yes | `"exp": 1686106200` |
| `merchant_id` | Merchant identifier - Allows a partner to route requests to a specific merchant. Useful when integrating multiple merchants under a single account. | No | `"merchant_id": "merchant_abc123"` |
...
$payload = [
    ...
    'merchant_id' => 'merchant_abc123', // Optional: specify the merchant
];
...
4. **Validation**: The server validates:
   ...
   - If `merchant_id` is provided, that it corresponds to a valid merchant under your account
...
3. We'll associate it with your account
...
- If using `merchant_id`, verify the ID matches a merchant registered under your account
