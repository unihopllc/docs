...
| `merchant_id` | Merchant identifier - Allows a partner to route requests to a specific merchant. Useful when integrating multiple merchants under a single partner account. | No | `"merchant_id": "merchant_abc123"` |
...
4. **Validation**: The server validates:
   ...
   - If `merchant_id` is provided, that it corresponds to a valid merchant under your partner account
...
- If using `merchant_id`, verify the ID matches a merchant registered under your partner account
